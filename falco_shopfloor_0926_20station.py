...
class RequestTypes(object):
  ...
  FA_START_20 = RequestType(
      'nft2/Request', 'nft2/Response', '.OK' , 'STATION=FAT')
...
#adam++      'CQ_Monitor/Handshake', 'CQ_Monitor/HandShakeResp', '', None)
  ...
  ALL = [SMT_START, SMT_END,
         FA_START, FA_START_FAT, FA_START_20, HWID_COMPLETE, FA_END]
...
#adam++        board='PEPPY')
  ...
  def GetDeviceInfo(self, mlb_sn):
    ...
    ret['region'] = re.sub('ABF', 'fr', ret['region'])
    ret['region'] = re.sub('ABD', 'de', ret['region'])
    ret['region'] = re.sub('ABL', 'ca.hybrid', ret['region'])
    ret['region'] = re.sub('ABG', 'au', ret['region'])
    ret['region'] = re.sub('ABH', 'nl', ret['region'])
    ret['region'] = re.sub('UUW', 'nordic', ret['region'])
    ret['region'] = re.sub('AB4', 'sg', ret['region'])
    ret['region'] = re.sub('ACJ', 'in', ret['region'])
    ret['region'] = re.sub('UUF', 'my', ret['region'])
...
# 		cellular = 'Y'
    ...
    response = self._CallBackend(RequestTypes.FA_START_20, mlb_sn,
                                  (('STATION', 'FAT'),
                                   ('errorcode','PASS'),
